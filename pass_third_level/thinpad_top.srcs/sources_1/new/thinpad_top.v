...
//  );
// PLL分频示例
wire locked, clk_10M, clk_20M, cpu_clk, soc_clk,uart_clk;
pll_example clock_gen 
 (
  // Clock in ports
  .clk_in1(clk_50M),  // 外部时钟输入
...
wire is_base,is_ext,mem_oe,mem_we;
wire [3:0]      mem_be;
wire [19:0]     mem_addr;
...
assign base_ram_ce_n = ~is_base;
assign ext_ram_ce_n  = ~is_ext;
assign base_ram_be_n = ~mem_be;//低电平有效
assign ext_ram_be_n  = ~mem_be;
assign base_ram_oe_n = ~mem_oe|base_ram_ce_n;
assign ext_ram_oe_n  = ~mem_oe|ext_ram_ce_n;
assign base_ram_we_n = ~mem_we|base_ram_ce_n;
assign ext_ram_we_n  = ~mem_we|ext_ram_ce_n;
assign base_ram_addr = is_base?mem_addr:20'b0;
assign ext_ram_addr  = is_ext?mem_addr:20'b0;
assign mem_rdata = is_base?base_ram_data:ext_ram_data;
...
//         mem_Wdata <= mem_wdata;
// end
assign base_ram_data = is_base&&(~base_ram_we_n) ? mem_wdata : 32'bz;
assign ext_ram_data  = is_ext &&(~ext_ram_we_n)  ? mem_wdata : 32'bz;
